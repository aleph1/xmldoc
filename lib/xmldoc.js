(function () {
  ...
  function XmlElement(tag) {
    ...
    var root = delegates[delegates.length - 1];
    var readonly = root && root._readonly;
    ...
    if(readonly) {
      ...
      this.line = this.column = this.position = this.startTagPosition = undefined;
    }
  }

  // Private methods

  XmlElement.prototype._appendChild = function (child) {
  ...
  XmlElement.prototype._prependChild = function (child) {
    if(child.parent) child.remove();
    child.parent = this;
    // add to our children array
  ...
  XmlElement.prototype._removeChild = function (child) {
    ...
    if(childIndex !== -1) {
      ...
      if(this.children.length) {
        ...
        this.firstChild = this.lastChild = undefined;
      }
    }
    child.parent = null;
  };
  ...
  XmlElement.prototype._text = function (text) {
    ...
    this._appendChild(new XmlTextNode(text));
  };

  XmlElement.prototype._cdata = function (cdata) {
    this.val += cdata;

    this._appendChild(new XmlCDataNode(cdata));
  };

  XmlElement.prototype._comment = function (comment) {
    if (typeof this.children === "undefined") return;

    this._appendChild(new XmlCommentNode(comment));
  };
  ...
  XmlElement.prototype.eachChild = function (iterator, context) {
    ...
    for (var i = 0, l = childrenCopy.length; i < l; i++)
      if (childrenCopy[i].type === "element")
        if (
          iterator.call(context, childrenCopy[i], i, childrenCopy) === false
        )
          return;
  };
  ...
  XmlElement.prototype.append = function(...nodes) {
    for (var i = 0, l = nodes.length; i < l; i++) {
      this._appendChild(nodes[i]);
    }
  }

  XmlElement.prototype.prepend = function(...nodes) {
    for (var i = nodes.length - 1; i >= 0; i--) {
      this._prependChild(nodes[i]);
    }
  }
  ...
  // some extra code here as we don’t want the overhead of calling _removeChild for each element
  ...
  XmlElement.prototype.clone = function(deep) {
    var clone = new XmlElement({
      name: this.name,
      attributes: extend({},this.attr)
    });
    if(deep) {
      clone.val = this.val;
      this.eachChild(function(child){
        clone.append(child.cloneNode(deep));
      });
    }
    return clone;
  }
  ...
  XmlTextNode.prototype.clone = function(){
  ...
  XmlCDataNode.prototype.clone = function() {
    return new XmlCDataNode(this.cdata);
  }
  ...
  XmlCommentNode.prototype.clone = function() {
    return new XmlCommentNode(this.comment);
  }
  ...
  function XmlDocument(xml, options = {}) {
    ...
    this._readonly = options.readonly === true ? true : false;
  ...
  // THE FOLLOWING WOULD COMPRESS BETTER
  //var parserEvents = {
  //  onopentag: function(){
  //    delegates[0] && delegates[0]._opentag.apply(delegates[0], arguments);
  //  },
